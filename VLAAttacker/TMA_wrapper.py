...
def main(args):
    ...
    name = (
        f"{args.dataset}_{vla_path}_atk{args.attack_mode}_GA{args.accumulate}_lr{format(args.lr, '.0e')}_"
        f"iter{args.iter}_warmup{args.warmup}_filterGripTrainTo1{args.filterGripTrainTo1}_target{target}_"
        f"inner_loop{args.innerLoop}_geometry{args.geometry}_projection_size{projection_size}_seed42-{exp_id}"
    )
    ...
    openVLA_Attacker.patchattack_unconstrained(train_dataloader, val_dataloader, num_iter=args.iter,
                                               target_action=args.targetAction * np.ones(7), patch_size=args.patch_size, projection_size=projection_size, alpha=args.lr,
                                               accumulate_steps=args.accumulate,
