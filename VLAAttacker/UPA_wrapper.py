...
def main(args):
    ...
    name = (
        f"{args.dataset}_{vla_path}_atk{args.attack_mode}_reverse_direction{args.reverse_direction}_"
        f"GA{args.accumulate}_lr{format(args.lr, '.0e')}_iter{args.iter}_warmup{args.warmup}_"
        f"filterGripTrainTo1{args.filterGripTrainTo1}_target{target}_inner_loop{args.innerLoop}_"
        f"geometry{args.geometry}_projection_size{projection_size}_seed42-{exp_id}"
    )
