...
LAMBDA_ANTI_GT="${LAMBDA_ANTI_GT:-0.05}"
LAMBDA_LOGIT_MARGIN="${LAMBDA_LOGIT_MARGIN:-0.05}"
...
WINDOW_ROLLOUT_PROBE_ENABLED_VALUE="${WINDOW_ROLLOUT_PROBE_ENABLED:-false}"
...
TRAIN_ANCHOR_HORIZON_ITERS_VALUE="${TRAIN_ANCHOR_HORIZON_ITERS:-50}"
DETERMINISTIC_ANCHOR_SAMPLING_VALUE="${DETERMINISTIC_ANCHOR_SAMPLING:-true}"
...
"${PYTHON_BIN_VALUE}" VLAAttacker/UADA_rollout_online_env_wrapper.py \
    ...
    --projection_scale_max "${PROJECTION_SCALE_MAX:-2.0}" \
    --projection_region "lower_half_fixed" \
    --projection_lower_start 0.55 \
    --projection_width_ratio 1.00 \
