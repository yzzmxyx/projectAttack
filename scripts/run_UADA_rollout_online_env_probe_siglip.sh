...
PROBE_ROOT="${current_dir}/run/UADA_rollout_online_env_probe_siglip/${PROBE_ID}"
LOG_DIR="${PROBE_ROOT}/logs"
SUMMARY_CSV="${PROBE_ROOT}/probe_summary.csv"
mkdir -p "${LOG_DIR}"

# Historical SigLIP probe baseline used for fair comparisons against GT-SigLIP runs.
...
echo "SiGLIP probe root: ${PROBE_ROOT}"
...
run_variant() {
    ...
    python3.10 "${current_dir}/VLAAttacker/UADA_rollout_online_env_wrapper.py" \
        ...
        --bs 1 \
        --warmup 2 \
        --tags "UADA_rollout_online_env_probe_siglip" "${variant}" \
        --geometry true \
...
run_variant "siglip-only" "0.0" "0.0" "0.0" "0.0" "off" "1.0"
run_variant "action+siglip" "1.0" "0.0" "0.0" "0.0" "off" "1.0"
run_variant "siglip+ce" "0.0" "0.0" "0.0" "0.1" "pseudo_clean" "1.0"
run_variant "action+siglip+ce" "1.0" "0.0" "0.0" "0.1" "pseudo_clean" "1.0"
run_variant "history1+siglip" "0.0" "0.5" "0.0" "0.0" "off" "1.0"
run_variant "history1+siglip+ce" "0.0" "0.5" "0.0" "0.1" "pseudo_clean" "1.0"
run_variant "action+history1+siglip+ce" "1.0" "0.5" "0.0" "0.1" "pseudo_clean" "1.0"
run_variant "action+siglip+history1" "1.0" "0.5" "0.0" "0.0" "off" "1.0"

echo "SiGLIP probe summary written to: ${SUMMARY_CSV}"
