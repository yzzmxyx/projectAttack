...
cat > "${SUMMARY_CSV}" <<'EOF'
order_idx,variant,exp_id,run_dir,window_rollout_probe_enabled,window_rollout_metric_mode,window_phase_name,window_start_step,window_end_step,window_rollout_future_horizon,window_rollout_exp_base,lambda_action_gap,lambda_siglip,lambda_ce,lambda_window_rollout_loss,online_ce_mode,action_gap_mode_active,final_val_gt_action_gap,final_val_siglip_distance,final_val_ce_objective,final_val_window_rollout_metric_value,final_val_window_rollout_delta_weighted,final_val_window_rollout_delta_weighted_loss,final_val_window_rollout_clean_gt_action_gap,final_val_window_rollout_adv_gt_action_gap,final_val_total_probe_score,final_val_total_rollout_score,final_val_done_rate,final_val_episode_len
EOF
...
run_variant() {
    ...
    python3.10 "${current_dir}/VLAAttacker/UADA_rollout_online_env_wrapper.py" \
        ...
        --bs 1 \
        --warmup "${WARMUP:-2}" \
        --tags "UADA_rollout_online_env_probe_window_rollout_adv_gt" "${variant}" \
        --geometry true \
...
echo "Window rollout adv-gt probe finished: ${PROBE_ROOT}"
