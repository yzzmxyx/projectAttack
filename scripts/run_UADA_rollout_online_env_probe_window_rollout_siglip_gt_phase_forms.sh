...
run_variant() {
    ...
    "${PYTHON_BIN}" "${current_dir}/VLAAttacker/UADA_rollout_online_env_wrapper.py" \
        ...
        --bs 1 \
        --warmup "${WARMUP:-2}" \
        --tags "UADA_rollout_online_env_probe_window_rollout_siglip_gt_phase_forms" "${loss_family}" "${form_name}" "${variant}" \
        --geometry true \
...
window_metric_mode = str(data.get("window_rollout_metric_mode", "delta_weighted"))
...
echo "Window rollout SigLIP/GT phase-form probe finished: ${PROBE_ROOT}"
